...
public class Compile2Data extends HaikuVM {
    ...
    private void includeThis(String include) throws IOException {
        ...
        File cf=new File(HaikuDefs.getProperty("APP_BASE")+"/"+Haikufy.choice("haikuJava")+"/"+include+".h");
        if (!singleFile) {
		...
		for (int k = 0; k < list.length; k++) {
			...
			if (line.matches("^\\d+:.*")) {
				...
				if (start==0) {
					...
					outc.printf("%d+%d +2, %4d, %4d,    // max_stack, purLocals, purParams\n", code.getMaxStack(), code.getMaxLocals(), code.getMaxLocals()-getParams(method), getParams(method));
					outc.printf("\n");
					if (methodName.endsWith("_clinitHaikuMagic_V")) {
                        conditions.add(Condition.CLINIT.desc("found clinitHaikuMagic() in: "+classname));
						for (Clinit desc : Clinit.set()) {
				...
				if (token.length<=2 || !supressExceptionSupport(token[2])) {
	...
	private String compileJ2Image(String jcode, byte bc[], int start, int end, int[] remap) throws IOException {
		...
		case 3:
			...
			if (token[1].equals("invokevirtual")) {
				...
				ccode+=new Formatter().format("B(%d), LB(%s), ", getMessageParamNo(jcode), meth);
				break;
			}
			if (token[1].equals("invokespecial") || token[1].equals("invokestatic")) {
				...
				if (functionTable.isShortcut(meth)) {
					bcode=null;
					ccode="OP_INVOKESHORT_"+meth+", ";
					totalBClength-=2;
				} else {
					ccode+=new Formatter().format("ADR(%s), ", meth);
				}
				break;
			}
			...
			if (token[1].equals("putstatic") || token[1].equals("getstatic")) {
				...
				bcode+="_"+btype;
				ccode=new Formatter().format("%-20s ",  "OP_"+bcode+", ").toString();
...
//                        ccode+=new Formatter().format("NStructADR(%s), ", token[2].replaceFirst("(.*)[.](.+)", "$2"));
			...
			if (token[1].equals("iinc")) {
				...
				if (inc==1) {
					bcode="IINC1";
					ccode="OP_IINC1, "+new Formatter().format("B(%d), ", rm);
				} else {
...
//                    ccode+=new Formatter().format("B(%s), ", size(token[2]));
				    ...
				    // Which is: forces it to (anonymous) <java.lang.Object>
                    ccode+=new Formatter().format("B(%s), ", "0"); 
				} else {
					// 3:    new		<Fibonacci> (17)
					String adr=token[2];
					if (token.length>4) adr+="_"+token[3];
			...
			ccode+=new Formatter().format("INT16(%d), ", (short)((bc[start+1]&0xff)<<8) + (bc[start+2]&0xff));
			break;
		case 6:
			if (bcode.equals("IINC")) {
				...
				bcode="WIINC";
				ccode="OP_WIINC, "+ new Formatter().format("INT16(%d), INT16(%d), ", rm, (short)((bc[start+4]&0xff)<<8) + (bc[start+5]&0xff));
				break;
			}
			break;
		default:
			if (token[1].equals("invokeinterface")) {
				...
				ccode+=new Formatter().format("B(%s), B(%d), ", size(token[2]), bc[start+3]&0xff);
			} else {
				for (int i = 1; i < len; i++) {
	...
	private String compileJ2Typedef(String jcode, byte bc[], int start, int end) throws IOException {
		...
		case 3:
			...
			if (index!=null) {
				if (token[1].startsWith("ldc")) {
					...
					// 3:    anewarray		<java.lang.Object> (3)
					ccode="OP_bc op"+start+"; OP__8 s"+start;
				} else {
					// 3:    new		<Fibonacci> (17)
					ccode="OP_bc op"+start+"; OPadr f"+start;
				}
		...
		default:
			if (token[1].equals("invokeinterface")) {
				...
				// 11:   multianewarray	<[[[I>	3 (114)
				ccode="OP_bc op"+start+"; OP__8 s"+start+"; OP__8 d"+start;
			} else {
				ccode="OP_bc op"+start;
				for (int i = 1; i < len; i++) {
...
//                  throw new IndexOutOfBoundsException("int16_t size ecxeeded by "+((ConstantInteger)cons[i]).getBytes());
                    Verbose.warning("int16_t size ecxeeded: %d (0x%x)", ((ConstantInteger)cons[i]).getBytes(), ((ConstantInteger)cons[i]).getBytes());
                }
    		}
