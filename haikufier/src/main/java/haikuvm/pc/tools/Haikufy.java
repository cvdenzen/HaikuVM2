...
public class Haikufy {
	...
	static boolean linkerOption=true;
	 ...
	 */
	Hashtable<String, String> staticMap= new Hashtable<String, String>();
	
	static String exceptionTable="";
	static FunctionTable functionTable= new FunctionTable();

	protected static Vector<String> classnames;
	...
	protected static void includes(PrintOnChange haikuConfigc2) {
        if (singleFile) {
            if (!hasWrittenHaikuConfig) {
                haikuConfigc2.println("#include \"haikuConfig.h\"");
                haikuConfigc2.println("#include \"haikuJava.h\"");
                haikuConfigc2.println();
                hasWrittenHaikuConfig=true;
            }
        } else {
            haikuConfigc2.println("#include \"haikuConfig.h\"");
            haikuConfigc2.println();
        }
//        out.println("// just a placeholder for potential user definitions");
//        out.println("#include <haikuUserCode/extern.h>");
//        out.println();
	}
	...
	protected String calcTypedef() throws IOException {
		...
		if (list.size()>0) {
			...
			for (int i = 0; i <list.size(); i++) {
				TypedefInfo t=list.get(i);
				res+="  "+signature2JNI(t.sig)+ " "+mangleField(t.info)+";	//"+t.sig+"\n";
			}
	...
	private void calcFields(JavaClass localjc, int level, Vector<TypedefInfo> list) throws IOException {
		...
		for (int i = 0; i < fields.length; i++) {
			String[] token=fields[i].toString().split(" ");
			TypedefInfo t= new TypedefInfo();
			t.level=level;
			t.sig=fields[i].getSignature().toString();
			int vidx=token.length;
			if (fields[i].isFinal()) {
				for (vidx = 0; vidx < token.length; vidx++) {
					if (token[vidx].equals("=")) break;
				}				
			}
			t.var=token[vidx-1];
	...
	protected static String mangle(String java) {
		String mangled= java;
		...
		if(mangled.endsWith(mainOsThread.replace('.', '_').replace('/', '_')+"_throwException_V")) 
			mangled="throwException_V";
	 ...
	 */
	static String getMethodSignature(String jcode) {
...
//					}
	...
	/**
	 * Target.InternalExceptionThrow!=0 ==> let live<br>
	 * 
	 * Target.InternalExceptionThrow==0 && pattern.indexOf(".throwException")>=0 ==> wipe out
	 * 
	 * @param pattern
	 * @return
	 */
	public static boolean supressExceptionSupport(String pattern) {
		int es=0;
		if (HaikuDefs.getProperty("InternalExceptionThrow").startsWith("0x")) {
			es=Integer.parseInt(HaikuDefs.getProperty("InternalExceptionThrow").substring(2), 16);
		} else {
			es=Integer.parseInt(HaikuDefs.getProperty("InternalExceptionThrow"));
		}
		if (es!=0) return false;
		return pattern.indexOf(".throwException")>=0;
	}

    public static String choice(String path) {
        return "HaikuVM/utility";
    }
