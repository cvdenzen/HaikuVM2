...
#define HAIKU_NoGC 0
#define HAIKU_StopTheWorldGC 1
#define HAIKU_IncrementalGC 2
...
// enabled
#define VirtualMachineError			512
...
#define SADR(x) (OPadr)(&allStatics.statics.x)
...
#define	OP_GETSTATIC_NativeCVariable8 OP_GETSTATIC_B
#define	OP_PUTSTATIC_NativeCVariable8 OP_PUTSTATIC_B

#define	OP_GETSTATIC_NativeCVariable16 OP_GETSTATIC_S
#define	OP_PUTSTATIC_NativeCVariable16 OP_PUTSTATIC_S
...
#define	OP_GETSTATIC_NativeCVariable32 OP_GETSTATIC_I
#define	OP_PUTSTATIC_NativeCVariable32 OP_PUTSTATIC_I

#define	OP_GETSTATIC_NativeCVariable64 OP_GETSTATIC_J
#define	OP_PUTSTATIC_NativeCVariable64 OP_PUTSTATIC_J
	...
	typedef uint64_t jmillis; // see millis()
	typedef juint jheapsize;
#endif
...
#define	OP_PUTSTATIC_NativeCVariable64
	...
	typedef uint32_t jmillis; // see millis()
	typedef juint jheapsize;
#endif
...
#define	OP_GETSTATIC_NativeCVariable32 OP_GETSTATIC_J
#define	OP_PUTSTATIC_NativeCVariable32 OP_PUTSTATIC_J
...
//#define FREE ((jclass)-1) //!= 0 not needed because arrays are of Object as class (before NULL).
...
typedef struct {
	jint length;
	// WIN32: Leider ist bei folgender union sizeof(array_t)=12 statt wie erwartet 8 (Hat nichts mit alignment zu tun.)
//	union {
//		 jbyte jbyteArray[];
//		 jint jintArray[];
...
typedef union {
	...
	HVM_String * str;
	jfloat f;
	jint fl; // float: im Zusammenhang mit LDC und PROGMEM
	jint i;
	jboolean z; // boolean
	jbyte b;
	jchar c;
...
#define STACKIDX(frame, p) ((int)((unsigned int)(p)-(unsigned int)&frame->stack[0])/sizeof(stack_t))

typedef union {
	struct {stack_t s0; stack_t s1;};
	jdouble d;
	julong dl; // double: im Zusammenhang mit LDC und PROGMEM
	jlong j; // long
} top_t;
...
typedef struct {const OP_bc * from; const OP_bc * to; const OP_bc * handler; const jclass eclazz; } Exception_t;


#define currentThread ((native_java_lang_Thread*)allStatics.statics.java_lang_Thread_currentThread)


extern jstack dataSp;
extern jstack lsp;
...
{
#endif
extern stackFrame4Debug_t* stack;
extern stackFrame_t* newStack(int size);
...
extern array_t * newarray(jint length, int size);
extern void multiArray(jobjectArray* GCroot, int d, int size);
...
extern void unused();
extern void interpretNextByteCode();
extern void setupHaikuVM();
extern void startJVM();

extern void throwException(jint exception, jint arg);
extern char * getField();
extern const NativCode java_lang_System_gc_V;
extern void java_lang_System_gc_V_native();
extern jobject GCalloc(jclass clazz, jint size);
...
#else
#define GETCODEADR2() GETCODEWORD2()
#endif
...
extern const Exception_t	exceptionTable[];
extern const ByteCode *	functionTable[];
...
extern __attribute__((noinline)) void setMarkBit(jobject obj);
#else
...
#if	HAIKU_InternalExceptionThrow
extern jobject isExceptionThrown(jobject);
#else
#define isExceptionThrown(x) (x)
#endif
