 ...
 */
BEGCODE(ANEWARRAY)
	top.s1.a=isExceptionThrown(newarray(top.s1.i, GETCODEBYTE2()));
ENDCODE
...
BEGCODE(CHECKCAST)	
	...
	if (top.s1.a!=NULL && clazz!=getClass(top.s1.a)) {
		throwException(ClassCastException, getClass(top.s1.a));
	}
ENDCODE
...
BEGCODE(GETSTATIC_B)
	pushTop();
#if NO_MICRO
	top.s1.i=getMemory8((jbyte*)GETCODEADR2());
#else
...
BEGCODE(GETSTATIC_S)
	pushTop();
#if NO_MICRO
	top.s1.i=getMemory16((jshort*)GETCODEADR2());
#else
 ...
 */
BEGCODE(IDIV)
#if	HAIKU_InternalExceptionEnable & ArithmeticException
	if ( top.s1.i==0 ) {
		ipop();
		throwException(ArithmeticException, 0);
	} else {
 ...
 */
BEGCODE(IINC)
	jstack top_s1 =getLocal(GETCODEBYTE2());
 ...
 */
BEGCODE(INSTANCEOF)
	top.s1.i=instanceOf(top.s1.a, GETCODEADR2());
ENDCODE
...
BEGCODE(NEW)
	pushTop();
	top.s1.a=isExceptionThrown(newInstance((jclass)GETCODEADR2()));
ENDCODE
...
BEGCODE(PUTSTATIC_S)
#if NO_MICRO
	setMemory16((jbyte*)GETCODEADR2(), top.s1.s);
#else
 ...
 */
BEGCODE(WIINC)
	jstack top_s1 =getLocal(GETCODEWORD2());
