...
int getMemory16(uint8_t * mem_addr) {
	...
	switch (mem_addr-mem) {
		case ADC_IDX:	
		//AD-Wandlung getMemory16(ADC); // Ergebnis als 10-Bit-Wert
			v=(v+1)%1024;
			return v;
		default:
#if _DEBUG
...
__attribute__((noinline)) static int getSize(array_t* arr) {
	return (getObjLength((jheap)arr)-sizeof(array_t))/arr->length; // Hierhin wg. möglicher integer div by 0 (if dest->length==0)
}
...
void native_java_lang_Thread_fork_I() {
	...
	thisThread->stack=         (stackFrame4Debug_t*) newStack(HAIKU_InitialOtherThreadStackSize);
	thisThread->programcounter=pc;
	thisThread->stackframe=	(jstack) &thisThread->stack->stack[0]; 
	thisThread->stackpointer=	(jstack) &thisThread->stack->stack[1]; // stack[0] is not allowed because areturn() will trigger a (this time wrong) dequeue
	top.s1.i=1;
}
 ...
 * 
 * WARNING: haiku hack for this method:
...
OP_MONITOREXIT,                                                        // 1:    monitorexit
OP_GETSTATIC_L,      SADR(java_lang_Thread_currentThread),             // 2:    getstatic		java.lang.Thread.currentThread Ljava/lang/Thread; (9)
OP_BIPUSH,           B(6),                                             // 5:    bipush		6
OP_INVOKESHORT_java_lang_Thread_setStateAndSwitch_II,                  // 7:    invokespecial	java.lang.Thread.setStateAndSwitch (I)I (24)
OP_POP,                                                                // 10:   pop
OP_ALOAD_0,                                                            // 11:   aload_0
OP_MONITORENTER,                                                       // 12:   monitorenter
OP_RETURN,                                                             // 13:   return
};
#endif
...
filename		D:\Entwicklung\sourceforge.haikuvm\trunk\bootstrap\bin/java/lang/VM.class
