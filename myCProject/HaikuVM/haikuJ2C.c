...
const char * findFoo(unsigned char * pc) {
	ByteCode * foo, *tfoo;
	int k, fooIdx;
	foo=NULL;
	...
	if (foo==NULL || (unsigned int)pc-(unsigned int)foo>3000 ) return "<unknown function>";
	...
	if (stack==NULL) {
		...
		while(1) { // for each thread
			while (F!=NULL) { // for each stack (of this thread)
				...
				// some heuristiks:
				if (s==NULL || !isPointer((jheap)F) || F->length==0 || getAllocsize(F)!=sizeof(array_t)+F->length*pgm_read_byteRef(defs[8])) break;
...
jobject isExceptionThrown(jobject a) {
	if (allStatics.statics.haiku_vm_HaikuMagic_exceptionCode)	return top.s1.a;
	return a;
}

extern const ByteCode haiku_vm_MicroKernel_throwException_V;

/**
 * invoke(&haiku_vm_MicroKernel_throwException_V);
 * sets top.s1.a ! So don't overwirte it.
 */
void throwException(jint exceptionCode, jint exceptionArg) {
	// First: prevent recursive exceptions
	if (!allStatics.statics.haiku_vm_HaikuMagic_exceptionCode) {
		allStatics.statics.haiku_vm_HaikuMagic_exceptionCode=exceptionCode;
		allStatics.statics.haiku_vm_HaikuMagic_exceptionArg=exceptionArg;
		invoke(&haiku_vm_MicroKernel_throwException_V);
#if	_DEBUG
		showstack("throwException");
		haikuHeapWalk("heap", 1);
	} else {
		showstack("throwExceptionRecursiv");
		haikuHeapWalk("heap", 1);
#endif
	}
}
...
// see haikuJ2C.h #define isExceptionThrown(x) (x)

void throwException(jint exceptionCode, jint exceptionArg) {
	fatal("throwException", exceptionCode, exceptionArg);
}
#endif
...
#if	HAIKU_InternalExceptionThrow
		return &haiku_vm_MicroKernel_throwException_V;
#endif
	}
#endif
	{
...
secondTryAsObject:
		if(obj<(char*)&heapmem || ((char*)&heapmem)+HEAPSIZE<=obj) {
		...
		throwException(obj==NULL?NullPointerException:NoSuchMethodError, msgIdx);
#if	HAIKU_InternalExceptionThrow
		return &haiku_vm_MicroKernel_throwException_V;
#endif
		return NULL;
...
#if	HAIKU_InternalExceptionEnable & StackOverflowError
static int check(jstack p) {
	int sp=STACKIDX(stack, p);
	if (sp<0 || sp>=stack->length) {
		throwException(StackOverflowError, sp);
		return 1;
	}
	return 0;
}
...
int checkIndex(array_t * adr, jint index) {
	...
	if (index<0 || index>=adr->length) {
		throwException(ArrayIndexOutOfBoundsException, index);
		return 0;
	}
	return 1;
}
...
#if	HAIKU_InternalExceptionEnable & OutOfMemoryError
	    if (str==NULL) return;
#endif
...
void compare(jint v) {
	// Umstellung auf fast dispatch kostet leider 20 Byte und 100ms
	int8_t t;
...
void invoke(const ByteCode * bytecode){
	...
	//
	//bringt nichts (time(+3%) & space(+0.5%)):
	//  TaggedCode tc;
	...
	if((uint8_t)max_stack==0xff) {
		...
		// using int ist 10 bytes shorter than uint8_t
		//int purLocals=pgm_read_byteRef(bytecode->purLocals);
		int purParams=pgm_read_byteRef(bytecode->purParams);
		jstack nlsp;
		// L    s
		//...o123
		pushTop(); // TODO: Because evil if we are here caused by StackOverflow
		// L     s
		//...o123
...
//		if (stack->length-nlsp<=max_stack) {
		if (&stack->stack[stack->length]<=nlsp+max_stack) {
			...
			//if (m<40) m=40;
			int i;
			stackFrame_t * newstack=newStack(2+max_stack);
#if	HAIKU_InternalExceptionEnable & OutOfMemoryError
			if (newstack==NULL) {
				throwException(OutOfMemoryError, sizeof(array_t)+(2+max_stack)*pgm_read_byteRef(defs[8]));
				return;
			}
#endif		
