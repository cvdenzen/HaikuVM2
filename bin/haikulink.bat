...
:winnt
    ...
    echo #############################################################
    rem carl
    set debugpars= -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=5006 
    "%JAVA%" "-Dhaikuvm.home=%NXJ_HOME%" "-DCOMMAND_NAME=%NXJ_COMMAND%" -classpath "%NXJ_CP_PC%" %debugpars% haikuvm.pc.tools.HaikuVM --bootclasspath "%CLASSESDIR%;%NXJ_CP_NXT%" %*
